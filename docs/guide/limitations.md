...
```tsx
// сhanging options or functions after mount has no effect
<KanjouProvider locale={locale} messages={messages} options={options} functions={fns}>
```

We made this performance choice intentionally. The options object relies on an empty dependency array to prevent unnecessary rendering cycles. We might revisit this behavior in future releases.
...
The library currently does not print warnings to the console. We plan to improve this developer experience soon.
...
Not every project requires rich text capabilities. We split this into a separate provider to keep your production bundles as small as possible.
